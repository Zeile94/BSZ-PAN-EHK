...
// Neuer Endpunkt für Datei-Uploads inklusive Base64 Verarbeitung
app.post("/api/chat/file", async (req, res) => {
  try {
    ...
    // Datei mit multipart/form-data an OpenAI Files-API uploaden
    ...
    // Compose chat messages including file reference per OpenAI specs (example)
    const messagesToSend = [
      { role: "system", content: systemMessage },
      { role: "user", content: content || "" },
      {
        role: "user",
        content: `Ich habe dir eine Datei hochgeladen (${fileName}). Bitte analysiere diese und beantworte meine Frage im Kontext dazu.`,
        file: { file_id: fileId }
      }
    ];

    // Chat Completion mit Datei anfragen
